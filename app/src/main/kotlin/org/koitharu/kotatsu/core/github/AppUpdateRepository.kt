	...
	suspend fun getAvailableVersions(): List<AppVersion> {
		...
		return jsonArray.mapJSONNotNull { json ->
			...
			val asset = assets?.find { jo ->
				...
				val assetName = jo.optString("name")
				val matches = contentType == CONTENT_TYPE_APK || assetName.endsWith(".apk", ignoreCase = true)
				android.util.Log.d(
					"UPDATE_DEBUG",
					"  Checking asset '$assetName': content_type='$contentType', apk=$matches",
				)
				matches
			}
