...
abstract class BasePreferenceFragment(@StringRes private val titleId: Int) :
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		...
		view.setBackgroundResource(R.drawable.bg_taru_home_root)
		listView.clipToPadding = false
		listView.setBackgroundColor(ContextCompat.getColor(requireContext(), android.R.color.transparent))
		listView.updatePadding(top = resources.getDimensionPixelOffset(R.dimen.margin_normal))
		applyTarumiPreferenceLayout(preferenceScreen)
	}
	...
	private fun applyTarumiPreferenceLayout(group: PreferenceGroup?) {
		if (group == null) {
			return
		}
		for (i in 0 until group.preferenceCount) {
			val pref = group.getPreference(i)
			if (pref.key != "tarumi_about_card" && pref !is PreferenceCategory && pref.layoutResource !in TARUMI_CUSTOM_PREFERENCE_LAYOUTS) {
				pref.layoutResource = R.layout.preference_tarumi_item
			}
			if (pref is PreferenceGroup) {
				applyTarumiPreferenceLayout(pref)
			}
		}
	}
	...
	private companion object {
		...
		val TARUMI_CUSTOM_PREFERENCE_LAYOUTS = setOf(
			R.layout.preference_tarumi_about_card,
			R.layout.preference_memory_usage,
			R.layout.preference_slider,
			R.layout.preference_split_switch,
			R.layout.preference_theme,
			R.layout.preference_toggle_header,
		)
	}
}
