...
class FavoriteDialogViewModel @Inject constructor(
	...
	private var statusCategoryIds = emptySet<Long>()
	...
	val content = combine(
		...
		val statusCategories = favouritesRepository.ensureTarumiStatusCategories(categories)
		statusCategoryIds = favouritesRepository.getStatusCategoryIds(includeLegacy = true)
		mapList(statusCategories, tracker)
	}.withErrorHandling()
	...
	fun setChecked(categoryId: Long, isChecked: Boolean) {
		launchJob(Dispatchers.Default) {
			if (isChecked) {
				favouritesRepository.setStatusCategory(categoryId, statusCategoryIds, manga)
			} else {
	...
	private suspend fun mapList(categories: List<FavouriteCategory>, tracker: Boolean): List<ListModel> {
		...
		val hasStatus = categories
			.filter { it.id != FavouritesRepository.TARUMI_REMOVE_CATEGORY.id }
			.any { (cats[it.id]?.size ?: 0) > 0 }
		return categories.map { cat ->
			MangaCategoryItem(
				category = cat,
				checkedState = if (cat.id == FavouritesRepository.TARUMI_REMOVE_CATEGORY.id) {
					if (hasStatus) MaterialCheckBox.STATE_UNCHECKED else MaterialCheckBox.STATE_CHECKED
				} else {
