...
abstract class SuggestionDao : MangaQueryBuilder.ConditionCallback {
	...
	@Transaction
	@Query("SELECT manga.* FROM suggestions LEFT JOIN manga ON manga.manga_id = suggestions.manga_id WHERE manga.title LIKE :query OR manga.alt_title LIKE :query ORDER BY suggestions.relevance DESC LIMIT :limit")
	abstract suspend fun searchMangaByTitle(query: String, limit: Int): List<MangaWithTags>
